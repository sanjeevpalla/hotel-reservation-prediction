...
class DataIngestion:
    def __init__(self,config):
        self.env = config["data_source"]["env"]
    ...
    def copy_csv_from_local(self):
        try:
            if os.path.exists(self.file_name):
                ...
                raise CustomException(f"File {self.file_name} does not exist in the local directory", None)

        except Exception as e:
            logger.error("Error while copying the csv file from local")
            raise CustomException("Failed to copy csv file from local ", e)
    ...
    def run(self):
        ...
        try:
            ...
            if self.env == "gcp":
                ...
                logger.info("Local environment detected, skipping GCP download")
                self.copy_csv_from_local()
            self.split_data()
